...
const BookDetail = ({ stateLogin, statePreview }) => {
  ...
  return (
    <div className="book-detail">
      {stateModal && (
        <BooksForm btonName={"Update"} setStateModal={setStateModal} />
      )}
      ...
      {stateLogin && (
        <h1
          className={`view-account ${
            stateLogin && statePreview ? "title-client" : ""
          }`}
        >
          {stateLogin && statePreview ? <RiUser3Line /> : <RiSettings5Fill />}{" "}
          {stateLogin && statePreview ? "Client View" : "Administrator View"}
        </h1>
      )}
      <div className="container-details-ult">
        <img src={bookDetailed[0].srcImage} alt="Book BIG" />
        <div className="container-details">
          <div className="info-detail-container">
            <p>
              <span className="detail-key">Title:</span> {bookDetailed[0].title}
            </p>
            <p>
              <span className="detail-key">Author:</span>{" "}
              {bookDetailed[0].author.map((aut) => {
                return aut + ", ";
              })}
            </p>
            <p>
              <span className="detail-key">Year:</span>{" "}
              {bookDetailed[0].yearOfPublication}
            </p>
            <div className="description-box">
              <p>
                <span className="detail-key">Description:</span>{" "}
                {bookDetailed[0].description}
              </p>
            </div>
            <p>
              <span className="detail-key">ISBN:</span> {bookDetailed[0].isbn}
            </p>
            {/* <p>id: {id}</p> */}
          </div>
